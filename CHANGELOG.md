...
- **Presets are ticked, not copied into a chain.** The dialog behind the wand
  ...
  changes. The set is shared, and each job takes only the rules aimed at what
  it produces — tag rules for imports and FROM NAME, file-name rules for
  RENAMER — so a renaming preset never touches the tags FROM NAME reads. Each
  job's existing chain carries over as one of your presets, ticked where it
  was doing something. (#392)
...
- **The traffic lights sit at row 1's exact vertical center with a
  symmetric top/left inset and a little breathing room from the window's
  top edge, the brand mark reads as an actual brand mark next to the
  mode tabs instead of smaller text beside them, and the lights-to-brand
  gap now matches the brand-to-tabs gap.** Rather than fight CSS to match
  wherever macOS puts the buttons by default, `trafficLightPosition` in
...
- **The top bar shows the same /tagrex/ wordmark as the repo page.** It was
  ...
  outlines in the theme's accent colour, centred on the window buttons by
  the height of its slashes. (#401)
...
- **The Online media badge is a rounded rectangle, not a capsule.** #334 gave it
  ...
  so the badge now uses `--radius-sm` like them, keeping the dark scrim, white
  16 px glyph and count. (#335)
- **The Online media badge is a dark scrim.** Following #333, the media-type
  ...
  dark translucent fill with a white glyph — after the SwiftUI stand's badge — so
  the cover art reads through and the glyph stays legible over light, dark and
  busy art alike. (#334)
